#sra-toolkit,fastqc, hisat2, samtools, stringtie, DESeq2
...
QualityFilter = function(fq1=fq1,fq2=fq2, # Input fq files. Set fq2="None" if single-end.
  ...
  # 1. Remove Adaptor; 2. low quality head/tail; 3. threshold of read length; 4. threshold of average quality score
...
# In this way, transcript IDs in gff will be used as protein IDs in faa.
...
enrichGO = function(Background=Background, # Tabular table with header and row names.
  ...
  TermOfDEGs = unname(unlist(sapply(DEGs2Term[,"term"],strsplit,split=","))); TermOfDEGs = sort(unname(TermOfDEGs[which(TermOfDEGs!="-")]))
...
enrichKEGG = function(Background=Background, # Tabular table with header and row names.
  ...
  Pathways = read.table(WholeGenomePath,sep="\t",header=TRUE,quote="");if ("MinPath"%in%colnames(Pathways)){Pathways = subset(Pathways,Pathways$MinPath!=0)}
  if (up){
    DEGs = rownames(subset(Background,color=="up"))
  }else{
    DEGs = rownames(subset(Background,color=="down"))
  ...
  Pathway2KO=read.table(Pathway2KO,sep="\t",header=FALSE,
                        quote="",colClasses="character",
                        col.names=c("Pathway","KO"))
  t=function(i){return(paste("map",i,sep=""))}
  Pathway2KO$Pathway=sapply(Pathway2KO$Pathway,t)
  KO2Des=read.table(KO2Des,sep="\t",header=FALSE,quote="",
                    col.names=c("KO","KO_Description"))
  ...
  DEGs2KO = data.frame(DEG=rep(DEGs2KO$query,time),
                       KO=unlist(KO))
