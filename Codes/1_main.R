...
# NGS RNA-seq data quality check
# Dependencies: FastQC
...
# Build hisat2 index of reference genome.
# Dependencies: Hisat2
...
# Extract gene/transcript read count matrix from output of stringtie.
# prepDE.py3 provided by StringTie
...
DESeq = function(mat=mat, # Expression matrix generated by stringtie-prepDE.py3.
                 metadata=metadata, # A non-header tabular table whose 
                                    ...
                                    # List experiment group prior to control group.
                 output_prefix=output_prefix){
  library(DESeq2)
...
# Dependencies: ComplexHeatmap (R), circlize (R)
Heatmap = function(mat=mat, # Expression matrix transformed by vst.
    ...
    pdf(output,onefile = FALSE);print(p);dev.off()
    return(p)
}

# CDS functional annotation.
# Dependencies: eggNOG-mapper
eggNOG = function(cds.fna=cds.fna,
                  out_prefix=out_prefix,
                  threads=threads,
                  block_size=block_size){
  threads=as.character(threads)
  block_size=as.character(block_size)
...
# GO enrichment
# Dependencies: annotate (R), stringr (R), dplyr (R)
...
# Dependencies: MinPath, stringr (R), dplyr (R)
...
# Dependencies: stringr (R), dplyr (R)
...
# Dependencies: pathview (R), stringr (R), dplyr (R)
