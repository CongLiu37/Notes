...
for bf in of:
    for l in open(bf):
        ...
        if not chrFiles.has_key(k):
            File='%s_%s_blastHits' % k
            cmd="echo '"+'\t'.join(['#query', 'chr', 'ident', 'overlap', 'del', 'insert', 'strand','query_start', 'query_end', 'query_len', 'chr_start', 'chr_end', 'eval', 'score'])+"' >> "+File
            os.system(cmd)
            #Files=open('%s_%s_blastHits' % k, 'a')
            #Files.write('\t'.join(['#query', 'chr', 'ident', 'overlap', 'del', 'insert', 'strand','query_start', 'query_end', 'query_len', 'chr_start', 'chr_end', 'eval', 'score'])+'\n')
        ...
        #chrFiles[k].write('\t'.join(f[:QueryStart]+[strand]+f[QueryStart:ChrStart]+[queries[f[0]]]+f[ChrStart:])+'\n')
        cmd="echo '"+'\t'.join(f[:QueryStart]+[strand]+f[QueryStart:ChrStart]+[queries[f[0]]]+f[ChrStart:])+"' >>"+File
        os.system(cmd)
        #Files.write('\t'.join(f[:QueryStart]+[strand]+f[QueryStart:ChrStart]+[queries[f[0]]]+f[ChrStart:])+'\n')
        #Files.close()
        #chrFiles[k].close()
        ...
        EValX = EVal+3
        cEndX = ChrEnd+3
        print('sort -k %d,%dn -k %d,%dg -k %d,%dnr %s | uniq >> %s.sorted'% (cStartX, cStartX, EValX, EValX, cEndX, cEndX, k, k))
        os.system('sort -k %d,%dn -k %d,%dg -k %d,%dnr %s | uniq > %s.sorted'% (cStartX, cStartX, EValX, EValX, cEndX, cEndX, k, k))
...
#    print('sort -k %d,%dn -k %d,%dg -k %d,%dnr %s > %s.sorted'
